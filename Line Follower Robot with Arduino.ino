...
void loop()
{
  ...
  if (millis() > timeToStop)
  {
    ...
    */
    ...
    lineFollower(5,10,30,50,40);
  
}
  

// Make the robot track to the line.
void lineFollower(int kKp, int kKi, int kKd, int speed_L, int speed_R)
{
  lineCheck();
  PID(kKp,kKi,kKd);
...
void lineCheck()
{
  ...
  if ( millis() - timeLSensor >= debounceDelayLSensor )
  {
    ...
    if (s==0 && t==0 && u==0 && v==0 && w==0 && x==0 && y==0 && z==0)
    {
      // Setup a timer
      if (timeStopB > 0)
    ...
    else
    {
      ...
      Position = ( 2*s + 8*t + 12*u + 14*v + 16*w + 18*x + 22*y + 28*z ) / totalPosition;
    }
